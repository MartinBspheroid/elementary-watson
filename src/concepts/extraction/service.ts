...
export class ExtractionService {
	 ...
	 */
	private searchInTranslations(obj: Record<string, unknown>, text: string, prefix = ''): string | null {
		for (const [key, value] of Object.entries(obj)) {
			...
			if (typeof value === 'string' && value === text) {
				...
				const nestedResult = this.searchInTranslations(value as Record<string, unknown>, text, currentKey);
				if (nestedResult) {
					return nestedResult;
				}
			}
	 ...
	 */
	async getUserInterpolationChoice(languageId: string, keyName = 'key'): Promise<'template' | 'code' | null> {
	 ...
	 * @param value The translation value
	 */
	async updateLocaleFile(workspacePath: string, locale: string, key: string, value: string): Promise<void> {
		try {
