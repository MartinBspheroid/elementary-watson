...
export class SidebarService {
	...
	async navigateToKey(editor: vscode.TextEditor, key: string): Promise<void> {
		try {
			...
			const searchValue = translationValue.endsWith('*') ? translationValue.slice(0, -1) : translationValue;
		...
		try {
			...
			if (key.includes('.')) {
				...
				for (const m of matches) {
					...
					const containsParentKeys = keyParts.slice(0, -1).every((parentKey) => beforeText.includes(`"${parentKey}"`));
